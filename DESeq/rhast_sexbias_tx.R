...
output_all <-read.table("counts_exon_parent.txt", row.names=1, header=T, skip =1, quote="")
colnames_new <- str_replace(colnames(output_all), 
                            "X.ohta1.bianca.sacchi.rhast_remap.JoshPop.STAR.Picard.AnalysisReady_\\d{1,2}.",
                            '') %>% 
  str_replace(., "_.bam","")
...
output_filtered <- output_all %>%
  dplyr::filter(., rowSums(across(TM4:TM3))>=20) %>% 
  rownames_to_column("GeneID")
head(output_filtered)
write.csv(output_filtered$GeneID, "expressed_genes_leaf.csv")
coldata <- read.csv("../rhast_DE_june_2021/coldata_june2021.csv") 
...
counts_tx <- output_filtered %>% select(starts_with("T")) 
coldata_tx <- coldata %>% dplyr::filter(grepl("T", Sample))
...
##
write.csv(sexDEGS_tx,"sex_biased_leaf_TX.csv", quote =F)
